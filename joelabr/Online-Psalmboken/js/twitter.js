 ...
 *           count: {Int} 1-20, defaults to 1 - max limit 20
             filter {Regex}: Used to select which tweets to display.
...
if (typeof renderTwitters != 'function') (function () {
    ...
    window.renderTwitters = function (obj, options) {
        ...
        for (i = 0, numRendered = 0; numRendered < max && obj[i]; i++) {
            ...
            if(data["text"].match(options.filter) == null)
    ...
    function getTwitterData(orig) {
        ...
        data.time = toDate(orig.created_at);
