...
GirderMarker.prototype.masterPos = function () {
  ...
  masterPos.top = function() {
    if ( Math.abs( cosine ) > 1 - EPSILON ) return new Phaser.Point( masterPos.x + cosine, masterPos.y - cosine * 32 );
    else return new Phaser.Point( masterPos.x + sine * 32, masterPos.y + sine );
  }

  masterPos.front = function() {
    if ( Math.abs( cosine ) > 1 - EPSILON ) return new Phaser.Point( masterPos.x + cosine, masterPos.y );
...
GirderMarker.prototype.roundTargetPos = function ( pos ) {
  return new Phaser.Point( Math.round( pos.x/32 ) * 32, Math.round( pos.y/32 ) * 32 );
}
...
GirderMarker.prototype.placeGirder = function () {
  ...
  if ( this.placeable ) {
    ...
    // make some particles!
    this.debrisBurst = new ParticleBurst( this.sprite.position.x, this.sprite.position.y, "Debris", {
      lifetime: 500,
      count: 14,
      scaleMin: 0.4,
      scaleMax: 1.0,
