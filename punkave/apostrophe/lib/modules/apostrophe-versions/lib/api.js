...
module.exports = function(self, options) {
  ...
  self.revert = function(req, version, callback) {
    ...
    return async.series({
      unversionedFields: function(callback) {
        return self.apos.callAll('docUnversionedFields', req, version._doc, unversionedFields, callback);
      }
    }, function(err) {
      if (err) {
        return callback(err);
  ...
  self.find = function(req, criteria, options, callback) {
    ...
    return async.series({
      ...
      permissions: function(callback) {
        ...
        var docId = versions[0].docId;
        return self.apos.docs.find(req, { _id: docId }).permission('edit').toObject(function(err, doc) {
          if (err) {
            return callback(err);
          }
          if (!doc) {
    ...
    function compareObjects(schema, version1, version2) {
      ...
      function compareField(field, old, current) {
        ...
        if (field.type === 'array') {
          ...
          var oldLines = [], oldText = '';
          var currentLines = [], currentText = '';
