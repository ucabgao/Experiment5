...
router.post('/iteration', function (req, res, next) {
    ...
    runSql("INSERT INTO iteration (name, date_retro_start, date_retro_end) VALUES($1, $2, $3);", [name, new Date(startDate), new Date(endDate)], function (err, results) {
        res.status(200).send({ 'Status': 'OK' });
    });
});
...
router.post('/results', function (req, res, next) {
    ...
    runSql("SELECT id FROM person WHERE email = $1", [email], function (err, results) {
        ...
        if (err || results.rows.length != 1) {
            ...
            runSql("SELECT id FROM iteration where date_retro_start < $1 AND date_retro_end >= $2", [date, date], function (err, results) {
                if (err || results.rows.length != 1) {
                    ...
                    runSql("INSERT INTO results (user_id, iteration_id, fit, proud, excited, meaningful, company) " +
                        "VALUES($1, $2, $3, $4, $5, $6, $7);", [user_id, iteration_id, fit, proud, excited, meaningful, company], function (err, results) {
                        res.status(200).send({ 'Status': 'OK' });
                    });
                }
            });
